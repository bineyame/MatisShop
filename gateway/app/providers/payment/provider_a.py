"""Payment Provider A - adapter seam awaiting its API specification.
...
6. record the field-by-field mapping in ``docs/providers/payment-provider-a.md``.

Rename this module and its registry key to the provider's real name once the
document arrives; ``provider_a`` is a placeholder label, not a product.
"""
...
class ProviderAPaymentProvider(PaymentProvider):
    name = "provider_a"
